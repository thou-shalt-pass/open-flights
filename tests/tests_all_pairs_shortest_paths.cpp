...
AdjMatrix EdgeListToAdjMatrix(const std::vector<std::pair<size_t, size_t> >& verticies, 
        const std::vector<unsigned>& weights, size_t n) {
...
TEST_CASE("all_pairs_shortest_paths(const adjMatrix& graph) Small Graph") {
  ...
  SECTION("3 Vertices Graph") {
    ...
    std::pair< std::vector<double>, std::vector<std::vector<size_t> > > all_distance_path = 
        AllPath(AllPairsShortestPaths(adjmatrix));
    std::vector<double> all_expect_distance { 1, 7, 6 };
    std::vector<std::vector<size_t> > all_expect_paths {
        { 0, 1 },
        { 0, 1, 2 },
        { 1, 2 }
    };   
  ...
  SECTION("5 Vertices Graph") {
    ...
    std::pair< std::vector<double>, std::vector<std::vector<size_t> > > distance_path = 
        AllPath(AllPairsShortestPaths(adjmatrix));
    std::vector<double> expect_distance { 6, 7, 4, 8, 1, 4, 2, 5, 1, 6 };
    std::vector<std::vector<size_t> > expect_paths {
        { 0, 1 },
        { 0, 1, 2 },
        { 0, 3 },
        { 0, 1, 4 },
...
TEST_CASE("all_pairs_shortest_paths(const adjMatrix& graph) Normal Graph") {
  SECTION("7 Vertices Graph") {
    ...
    std::vector<unsigned> weight { 6, 1, 3, 9, 10, 2, 2, 4, 7, 13 };
    std::vector< std::pair<size_t, size_t> > verticies { 
        { 0, 1 }, { 0, 2 }, { 1, 2 }, { 1, 4 }, { 2, 3 },
        { 3, 4 }, { 4, 5 }, { 3, 5 }, { 5, 6 }, { 3, 6 }
    };
  ...
  SECTION("8 Vertices Graph") {
    std::vector<std::tuple<size_t, size_t, unsigned> > edges;
    std::vector<unsigned> weight { 3, 7, 1, 2, 3, 1, 8, 9, 2, 7, 4, 7, 1 };
    std::vector< std::pair<size_t, size_t> > verticies {
        { 1, 2 }, { 2, 8 }, { 8, 7 }, { 7, 6 }, { 6, 5 },
        { 5, 4 }, { 4, 3 }, { 3, 1 }, { 1, 6 }, { 4, 8 },
        { 3, 7 }, { 2, 5 }, { 0, 1 }
